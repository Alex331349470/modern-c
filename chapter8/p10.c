...
int main(void)
{
    int i, user_time, hour, d_hour, d_minute, a_hour, a_minute, minute,
        dep[8] = {480, 583, 679, 767, 840, 945, 1140, 1305},
        arr[8] = {616, 712, 811, 968, 1075, 1280, 1438};

    printf("Enter a 24-hour hour time: ");
    ...
    for (i = 0; i < 7; i++) {
        if (user_time <= dep[i] + (dep[i + 1] - dep[i] / 2)) {
            ...
            printf("%d : %.2d %c.m., arriving at %d : %2.d %c.m.\n", d_hour, d_minute, dep[i] / 60 > 11 ? 'p' : 'a',
